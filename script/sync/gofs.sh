...
function gofs_image_exist() {
  if [ "$GOFS_IMAGE_NAME" = "$(sudo ctr image ls | grep $GOFS_IMAGE_NAME | awk '{print $1}')" ]; then
    return 0
  else
    return 1
  fi
...
function get_task_status() {
  sudo ctr t ls | grep $GOFS_CONTAINER | awk '{print $3}'
}

# 检查task是否存在
function gofs_task_exist() {
  if [ "$GOFS_CONTAINER" = "$(sudo ctr t ls | grep $GOFS_CONTAINER | awk '{print $1}')" ]; then
    return 0
  else
    return 1
  fi
...
function gofs_container_exist() {
  if [ "$GOFS_CONTAINER" = "$(sudo ctr c ls | grep $GOFS_CONTAINER | awk '{print $1}')" ]; then
    return 0
  else
    return 1
  fi
...
function gofs_pull_image() {
  if gofs_image_exist; then
    ...
    sudo ctr image pull $GOFS_IMAGE_NAME
  fi
}
...
function gofs_stop_task() {
  ...
  if [ "RUNNING" = "$GOFS_TASK_STATUS" ]; then
    sudo ctr t kill -s 9 $GOFS_CONTAINER
...
function gofs_start_task() {
  sudo ctr t start -d $GOFS_CONTAINER
}

# 删除task
function gofs_delete_task() {
  if gofs_task_exist; then
    sudo ctr t rm $GOFS_CONTAINER
...
function gofs_show_state() {
  ...
  sudo ctr t ls | grep $GOFS_CONTAINER
  sudo ctr c ls | grep $GOFS_CONTAINER
}
